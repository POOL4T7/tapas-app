...
const Imprint = ({ langData }:any) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 text-gray-800">
      <h1 style={{ color: '#800000' }} className="text-4xl font-bold mb-8 text-center">
        {title}
      </h1>
      ...
      {disputeResolution.details.map((detail:any, index:any) => (
        <p key={index} className="text-base leading-7 mb-2">
          {detail.platform ? (
            <>
              {detail.platform}{" "}
              <Link href={detail.link} target="_blank" rel="noopener noreferrer" className="text-blue-600">
                {detail.link}
              </Link>
            </>
          ) : (
      ...
      {liabilityForContent.details.map((detail:any, index:any) => (
        <p key={index} className="text-base leading-7 mb-2">{detail}</p>
      ))}
