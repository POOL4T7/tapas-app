...
const FacilityHead = ({ params }: { params: { ourFacilities: string } }) => {
  const headContent: Record<string, { title: string; description: string }> = {
    essbereich: {
      title:
        'Essbereich & Außenterrasse – Entspanntes Tapas-Erlebnis in Berlin',
      description:
    ...
    fruhstuck: {
      title:
        'Frühstück in der Mundo Tapas Bar | Frische Morgenaromen von Mo-Sa',
      description:
  ...
  const canonicalUrl = `https://alt-mariendorf.tapas-mundo.euen/einrichtungen/${params.ourFacilities}`;
