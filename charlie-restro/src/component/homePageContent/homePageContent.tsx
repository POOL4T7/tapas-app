...
export default function MundoIntro({content}:any) {
  ...
  return (
    <section style={{ backgroundImage: isMobile?`url(${isimg.mob})`:`url(${isimg.lap})`}} className={`${styles.main_container} py-12 px-4 md:px-12`}>
      <div className="max-w-5xl bg-[rgba(0,0,0,0.7)] p-4 py-8 rounded-lg mx-auto text-center">
        <h2 className="text-2xl py-4 font-bold text-[#fcca0c]">{content.title}</h2>
        ...
        {content.features.map((para:any, idx:any) => (
          <p key={idx} className="text-lg flex text-justify text-[white] space-y-0">
             <IoMdStar className="mt-1" color="#fcca0c"/>&nbsp;{para}
          </p>
        ))}
        </div>
