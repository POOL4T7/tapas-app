...
const CookieConsent= ({data}:any) => {
  ...
  return (
<div style={{ zIndex: '1' }} className="fixed bottom-0 left-0 right-0 bg-white text-black py-4 px-6 flex flex-col md:flex-row justify-center items-center text-xs md:text-sm">
  <p className="text-xs md:text-sm mb-2 md:mb-0 flex flex-col md:flex-row items-center">
    <div className='flex-col'>

    <p>
    {data.message}
    <button className='text-[##f5a524] mx-1 md:mx-1 cursor-pointer	' onClick={handleAccept}>
    {data.acceptButton}
    </button>
    ...
    <Link href={`${firstSegment}/datenschutzrichtlinie`} className="underline text-[##f5a524] ml-1 md:ml-2 cursor-pointer	">
     {data.pbtn} 
     </Link>.
    </p>
     </div>
  ...
  <button
    ...
    className="bg-[#f5a524] text-[black] px-3 py-1 rounded mt-2 md:mt-0 md:ml-4 text-xs md:text-sm cursor-pointer	"
  >
    {data.acceptButton}
  </button>
</div>
